...
- Scans service connections
- Scans execution history
- Scans YAML pipeline files from Git repo for service connection references
"""
...
def ado_get(url, headers):
    try:
        ...
        print(f"  WARNING HTTP {e.response.status_code}")
        return None
...
def deep_find_service_refs(obj, sc_names, found=None):
    if found is None:
        found = set()
    ...
    if isinstance(obj, dict):
        for k, v in obj.items():
            ...
            if lk in SERVICE_CONNECTION_INPUT_KEYS and isinstance(v, str):
                if v in sc_names:
                    found.add(v)
            deep_find_service_refs(v, sc_names, found)
    elif isinstance(obj, list):
        for item in obj:
            deep_find_service_refs(item, sc_names, found)
...
def scan_text_for_sc(text, sc_names):
    found = set()
    if not text:
        return found
    for sc in sc_names:
...
def get_yaml_file_path(pipeline_detail):
    """Extract YAML file path from pipeline detail JSON."""
    config = pipeline_detail.get("configuration", {})
    return config.get("path", "azure-pipelines.yml")

def get_repo_info(pipeline_detail):
    """Extract repository info from pipeline detail JSON."""
    config = pipeline_detail.get("configuration", {})
...
def main():
    ...
    pipelines_base = f"https://dev.azure.com/{org}"
    ...
    # 3. Pipelines (newer API) + YAML file scan
    print("Scanning YAML pipelines via Pipelines API + Git file content...")
    ...
    if pipelines_data:
        ...
        print(f"  Found {len(pipelines_list)} pipelines via Pipelines API")

        for pl in pipelines_list:
            pl_id = pl.get("id")
            pl_name = pl.get("name", "")
            pl_detail = ado_get(
                f"{base_url}/{project_name}/_apis/pipelines/{pl_id}?api-version=7.1",
                headers
            )
            if not pl_detail:
                continue

            yaml_path = get_yaml_file_path(pl_detail)
            repo_info = get_repo_info(pl_detail)
            repo_id = repo_info.get("id", "")
            repo_name = repo_info.get("name", "")

            # Get yaml file content from Git repo
            yaml_content = None
            if repo_id and yaml_path:
                # Try main branch first
                for branch in ["main", "master"]:
                    file_url = (
                        f"{base_url}/{project_name}/_apis/git/repositories/{repo_id}/items"
                        f"?path={yaml_path}&versionDescriptor[versionOptions]=0"
                        f"&versionDescriptor[versionType]=branch&versionDescriptor[version]={branch}"
                        f"&resolveGitRef=True&api-version=7.1"
                    )
                    file_data = ado_get(file_url, headers)
                    if file_data and file_data != "":
                        if isinstance(file_data, dict) and file_data.get("content"):
                            yaml_content = file_data.get("content")
                            break
                        elif isinstance(file_data, str):
                            yaml_content = file_data
                            break

            if yaml_content:
                found_yaml_refs = scan_text_for_sc(yaml_content, sc_names)
                for ref in found_yaml_refs:
                    pipeline_map_rows.append({
                        ...
                        "DetectionMethod": "YamlFileScan",
                        "YamlFile": yaml_path,
                        "LastSeen": ""
                    })
                    usage_by_sc.setdefault(ref, {"times": 0, "pipelines": set(), "last_used": ""})
                    usage_by_sc[ref]["pipelines"].add(pl_name)

    # 4. Build definitions still (for completeness)
    print("Scanning build definitions (legacy)...")
    ...
    # 5. Deduplicate pipeline map
    dedup = {}
    for row in pipeline_map_rows:
        key = (
            row["Project"], row["PipelineType"], str(row["PipelineId"]),
            row["PipelineName"], row["ServiceConnection"], row["DetectionMethod"], row["YamlFile"]
        )
        dedup[key] = row
    ...
    if pipeline_map_rows:
        ...
        with open(map_csv, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(["Project", "PipelineType", "PipelineId", "PipelineName", "ServiceConnection", "DetectionMethod", "YamlFile", "LastSeen"])
